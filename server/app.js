...
app.post('/items/enforce/', function (req, res) {
  let inventoryID = req.body.id
  try {
    connection.beginTransaction(async function (err) {
      if (err) {
        res.status(400)  // http status를 400~500번대 오류로 변경해주면 따로 error 메세지 안보내도 에러 알람 뜨는거 같은데
        res.send("400 Error")  // 그ㄹ머 에러 메세지를 구체적으로 해줄 필요가 있나? << 지금한정도만 해도 문제없나
      }
      const enforceItem = await executeQuery(`SELECT i.id as inventory_id, k.* FROM inventory i INNER JOIN knowledge k ON i.itemId = k.id where i.id = '${inventoryID}'`)
      const enforceProbability = await executeQuery(`SELECT probability, price FROM enforce where rarity = '${enforceItem[0].rarity}' AND level = '${enforceItem[0].level}'`)
      ...
      if (userMoney[0].holdings > 0) {
        ...
        if (randomNum >= 5) {
        ...
        else if (randomNum <= enforceProbability[0].probability) {
          const enforceItemId = await executeQuery(`SELECT * FROM knowledge where name = '${enforceItem[0].name}' AND level = ${enforceItem[0].level + 1}`)
          ...
          connection.commit((err) => {
            if (err) {
              ...
              res.send({ result: [enforceItemId, enforceItem[0].inventory_id] })
            }
          })
        }
        else {
