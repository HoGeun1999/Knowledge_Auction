...
import { sellState,sellItemCount, updateSellCount } from "./getItem.js"
...
function renderItemINFO(item, data) {
    return function () {
        ...
        itemInfoText.innerHTML = '아이템 정보창' + '<br>' + '지식:' + data[0].name + '<br>' + '희귀도:' + data[0].rarity + '<br>' + '가격:' + data[0].price
...
function renderInventoryBox() {
    ...
    try {
        ...
        for(let i=0; i < items.length; i++){
            let url = 'http://localhost:3000/inventoryItemGet/' + items[i].id
            const p =fetch(url)
        ...
        Promise.all(itemList).then((values) => {
            for(let i=0; i < values.length; i++){
                const arr = [values[i][0][0],values[i][1]]
                const inventoryItem = makeItem(arr)
...
export { renderItemINFO, onClickItem, renderInventoryBox, setIsfull,isFull, userMoney, updateSpecialRandomTicket,
